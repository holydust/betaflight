    ...
    case BARO_BMP581:
#if defined(USE_BARO_BMP580) || defined(USE_BARO_SPI_BMP580) || defined(USE_BARO_BMP581) || defined(USE_BARO_SPI_BMP581)
        {
            ...
            if (bmp5xxDetect(&defaultBMP5XXConfig, baroDev, &detectedChip)) {
                // Per Bosch BMP581 datasheet (and PX4's bmp581.h BMP581_CHIP_ID_PRIM/SEC),
                // BMP581 silicon ships with chip ID 0x50 (primary rev) or 0x51 (secondary
                // rev) — both are the same BMP581 product. PR #14925's strict split into
                // BMP580 (0x50) and BMP581 (0x51) misidentifies common BMP581 silicon as
                // BMP580; align with PX4 and treat both detected IDs as BMP581. Actual
                // BMP580 silicon is rare and uses the same protocol/driver anyway.
                if (detectedChip == 0x50 || detectedChip == 0x51) {
